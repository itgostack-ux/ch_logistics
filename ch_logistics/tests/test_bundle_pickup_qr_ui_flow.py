...
def run():
    ...
    try:
        ...
        # sellable leaf to its canonical path, so the names captured above are
        # stale the moment the stores exist. Re-read them from the stores.
        d1wh = frappe.db.get_value("CH Store", s1, "warehouse") or d1wh
        d2wh = frappe.db.get_value("CH Store", s2, "warehouse") or d2wh
