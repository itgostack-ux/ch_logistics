...
ch_logistics.DriverMap = class DriverMap {
	...
	async _plot_next_stop(trip) {
		// Reuse the existing logistics_api: get_trip_detail returns the trip
		// with stops (each carrying gps_lat / gps_lng / sequence / status).
		try {
			const r = await frappe.call({
				method: "ch_logistics.api.logistics_api.get_trip_detail",
				args: { trip },
			});
